...
export const EmployeeModal: React.FC<EmployeeModalProps> = ({
    ...
    if (editData) {
      setForm({
        ...
        NGAY_SINH: editData.NGAY_SINH
          ? (editData.NGAY_SINH as string).split("T")[0]
          : "",
  ...
  const setLegal = (k: string) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setLegalForm((f) => ({ ...f, [k]: e.target.value }));
  ...
  const handleSaveLegal = async () => {
    ...
    try {
      await api.updateEmployeeLegal({
        ...
        soNguoiPhuThuoc:
          legalForm.soNguoiPhuThuoc !== ""
            ? Number(legalForm.soNguoiPhuThuoc)
            : undefined,
      });
  ...
  return (
    <Drawer
      ...
      subtitle={
        isEdit ? `Mã NV: ${form.MA_NV}` : "Điền thông tin nhân viên mới"
      }
      icon={<Users size={18} />}
      size="md"
      footer={
        activeTab === "info" ? (
          ...
          <>
            ...
            <Btn loading={savingLegal} onClick={handleSaveLegal}>
    ...
    >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <FormField label="Phòng ban">
            ...
            >
              ...
              {departments.map((d) => (
                <option key={d.MA_PHG} value={d.MA_PHG}>
                  {d.TEN_PB}
                </option>
              ))}
            </select>
      ...
      {activeTab === "legal" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ℹ️ Thông tin này dùng cho mục đích tính thuế TNCN và chuyển khoản
            lương tự động.
          </div>
          <FormField label="Mã số thuế cá nhân (MST)">
            <input
              ...
              value={legalForm.maSoThue}
              onChange={setLegal("maSoThue")}
            />
          </FormField>
          <FormField label="Số tài khoản ngân hàng">
            <input
              ...
              value={legalForm.soTaiKhoan}
              onChange={setLegal("soTaiKhoan")}
            />
          </FormField>
          <FormField label="Tên ngân hàng">
            <input
              ...
              value={legalForm.nganHang}
              onChange={setLegal("nganHang")}
            />
          </FormField>
          <FormField label="Số người phụ thuộc (giảm trừ gia cảnh)">
            <input
              ...
              min="0"
              max="10"
              value={legalForm.soNguoiPhuThuoc}
              onChange={setLegal("soNguoiPhuThuoc")}
            />
          </FormField>
        </div>
      )}
